...
# print(n_lab[0], n_lab[1], n_lab[2])
# for z in range(n_lab.shape[0]):
#     if int(n_lab[z,-1]) >=60000:
#         print(n_lab[z][0])
MATR = MATR2[:,:128]
MATR = MATR.cpu()
LAB = LAB.cpu()
...
for i in range(len(Mean_Pos)):
    ...
    N_LAB.append(L_n_lab_i)
            ...
            # LAB_tract_i.append(LAB_2[L_i_128D[k]])
    # LAB_TRACT.append(LAB_tract_i)
print(len(L_D_128D_int))
print(L_D_128D_int)
# L_reject = sorted(L_reject)
# L_reject = list(np.unique(L_reject))
# print(len(LAB_tract_i))
# print(LAB_tract_i)
...
# print("nlab[0]", n_lab[0])
# print("nlab[1]", n_lab[1])
# file_name = f"L_n_lab_new_seuil_0.5.pkl"
# open_file = open(file_name, "wb")
# pickle.dump(to_save, open_file)
# open_file.close()

# for i in range(MATRT.shape[0]):
#     Dist_cluster_i = torch.zeros(57)
#     for j in range(MATRT.shape[1]):
#         Dist_i[,j] = (Mean_Pos[:,j]-MATRT[i,j])**2
#     Dist_i = torch.sum(Dist_i, dim=1)
#     Dist_i = torch.sqrt(Dist_i)
...
# MeanD = torch.tensor([])
# STDD = torch.tensor([])
# for p in range(len(L_D_128D_int)):
# #     print("before p ", len(L_D_128D_int[p]))
#     matrbi = MATRB[L_D_128D_int[p],:]
#     dist_b_i = torch.zeros(matrbi.shape[0], matrbi.shape[1])
#     for j in range(matrbi.shape[1]):
#         dist_b_i[:,j] = (Mean_Pos[p,j]-matrbi[:,j])**2
#     dist_b_i = torch.sum(dist_b_i, dim=1)
#     dist_b_i = torch.sqrt(dist_b_i)
# #     print(dist_b_i.shape)
#     Mean_dist = torch.mean(dist_b_i)
#     Std_dist = torch.std(dist_b_i)
#     MeanD = torch.cat((MeanD, Mean_dist.unsqueeze(0)), dim=0)
#     STDD = torch.cat((STDD, Std_dist.unsqueeze(0)), dim=0)
# #     L_D_128D_int_maj_i = []
# #     for b in range(len(dist_b_i)):
# #         if dist_b_i[b]<Mean_dist+Std_dist:
# #             # print(L_D_128D_int[p][b])
# #             L_D_128D_int_maj_i.append(L_D_128D_int[p][b])
# #     L_D_128D_int_maj.append(L_D_128D_int_maj_i)
# #     print("after p ", len(L_D_128D_int[p]))
# #     print("after p maj", len(L_D_128D_int_maj))
# # LL= []
# # for i in range(len(L_D_128D_int_maj)):
# #     LL.extend(L_D_128D_int_maj[i])
# # LL = sorted(LL)
# # LL = list(np.unique(LL))
# # print("nb_closest points maj", len(LL))
# # print(MATRT.shape)
# # MATRT = MATRT[LL,:]
# # print(MATRT.shape)
# # print(LAB_2.shape)
...
# # # for p in range(len(L_D_128D_int_maj)):
# # #     for a in range(len(L_D_128D_int_maj[p])):
# # # for z in range(57):
# # #     for i in range(len(L_D_128D_int_maj[z])):
# # #         LAB_2[L_D_128D_int_maj[z]] = z
...
#             # Dist = torch.sum((LIGHTS[i,a]-MATRT[j,a])**2)
#             # Dist = (LIGHTS[i,a]-MATRT[j,a])**2
...
for i in range(len(threedpca_lights)):
    ...
    # if i in missing_list:
        # l_2 = (LAB_2==i).nonzero().squeeze()
        # l_2 = [j.item() for j in l_2]
    # print(l_i)
    # print(LAB)
    # print(LAB[l_i])
    # print(LAB[l_i].shape)
