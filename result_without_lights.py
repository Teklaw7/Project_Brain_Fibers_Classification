...
# torch.save(Mean_Pos, "/CMF/data/timtey/Mean_Pos_128D_bundle.pt")
Mean_Pos = torch.load("/CMF/data/timtey/Mean_Pos_128D_bundle.pt")
...
for i in range(MATRT.shape[0]):
    ...
    if distance[min_dist]<0.7:
        L_accept.append(i)
print(len(L_accept))
torch.save(LAB_2, "/CMF/data/timtey/LAB_2_norm.pt")
file_name = "L_accept_128D_0.7_071823_norm"
open_file = open(file_name, "wb")
pickle.dump(L_accept, open_file)
open_file.close()
# open_file = open("L_accept_128D_0.7_071823", "rb")
# L_accept = pickle.load(open_file)
# open_file.close()
...
# tsne_result = tsne.fit_transform(GLOBAL)
# labeled_results = tsne_result[:MATRB.shape[0],:]
# tract_results = tsne_result[MATRB.shape[0]:,:]
print(labeled_results.shape)
print(tract_results.shape)
print(mean_pos_results.shape)
fig = plt.figure()
...
# print(uniqu_id.shape)
# for i in range(n_lab_subject_id.shape[0]):
    # print(n_lab_subject_id[i].item())
print("LAB_2", LAB_2.shape)
print("LAB_2 uniques", torch.unique(LAB_2))
