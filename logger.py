...
class BrainNetImageLogger_contrastive_tractography_labeled(Callback):
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):        
        ...
        if batch_idx % self.log_steps == 0:
            ...
            V1 = randomstretching(Vo.double()).to(pl_module.device,non_blocking=True)
            V2 = randomstretching(Vo.double()).to(pl_module.device,non_blocking=True)
            VFI1 = randomstretching(VFI.double()).to(pl_module.device,non_blocking=True)
            VFI2 = randomstretching(VFI.double()).to(pl_module.device,non_blocking=True)
            V1 = randomrot(V1).to(pl_module.device,non_blocking=True)
